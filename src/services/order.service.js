...
const createOrder = async (accessTokenFromHeader, items) => {
  ...
  try {
    ...
    throw new ApiError(httpStatus.INTERNAL_SERVER_ERROR, 'Error occurred when creating orders');
  }
};
...
const getOrderBySellerId = async (accessTokenFromHeader) => {
  try {
    ...
    const orders = await Order.find({ item: { $in: productIds }, $or: [{ status: 'Pending' }, { status: 'Refund' }] })
      .populate('item')
      .exec();
