...
const { Product, ProductList } = require('../models');
...
const createProduct = async (accessTokenFromHeader, imageList, productBody) => {
  const { productListId } = productBody;

  // get userId from token
  const payload = jwt.verify(accessTokenFromHeader, config.jwt.secret);
  const userId = payload.sub;

  const product = await Product.create({ ...productBody, category: productListId, sellerId: userId, images: imageList });
...
const searchProduct = async (keyword, options) => {
  const filter = {
    $or: [{ name: { $regex: `.*${keyword.keyword}.*`, $options: 'i' } }],
  };
  const products = await Product.paginate(filter, options);
  return products;
};
...
const updateProductById = async (accessTokenFromHeader, productId, updateBody) => {
  ...
  if (userId === product.sellerId) {
    const { productListId } = updateBody;

    const updatedProduct = {
      ...product,
      ...updateBody,
      category: productListId,
